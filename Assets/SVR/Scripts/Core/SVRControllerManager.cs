...
public class SVRControllerManager : MonoBehaviour
{
    ...
    void Start()
    {
        InputSystem.onDeviceChange += (device, change) =>
        {
            switch (change)
            {
                case InputDeviceChange.Added:
                    Debug.LogFormat(
                        "New device added: {0}, usage : {1}, path : {2}",
                        device,
                        device.usages,
                        device.path
                    );
        ...
        left_device = InputSystem.AddDevice(
        ...
        right_device = InputSystem.AddDevice(
            new InputDeviceDescription
            {
                interfaceName = "SVRInputDevice",
                product = "Surreal-Interactive"
        ...
        svr.SVRInputApi.SVRSetButtonCallback(
            Marshal.GetFunctionPointerForDelegate(
                (svr.SVRInputApi.ButtonCallbackDelegate)ButtonInputCallback
            )
        );
#endif
    ...
    void OnBeforeRender()
    {
        if (left_poll_timestamp_queue_.Count == queue_windows_size)
        {
            ...
            svr.SVRInputApi.SVRRenderFinish(
                svr.SVRInputApi.Chirality.Left,
                poll_timestamp,
                render_timestamp
            );
        }
        ...
        if (right_poll_timestamp_queue_.Count == queue_windows_size)
        {
            ...
            long render_timestamp = svr.SVRInputApi.SVRTimeNow();
            svr.SVRInputApi.SVRRenderFinish(
                svr.SVRInputApi.Chirality.Right,
                poll_timestamp,
                render_timestamp
            );
        }
        ...
        if (left_device != null)
        {
            ...
            left_state.tracking_state_ = svr.SVRInputApi.SVRIsConnected(0)
        ...
        if (right_device != null)
        {
            ...
            right_state.tracking_state_ = svr.SVRInputApi.SVRIsConnected(1)
                ? (InputTrackingState.Position | InputTrackingState.Rotation)
                : InputTrackingState.None;
    ...
    void SVRUpdatePose(long poll_timestamp, int chirality, ref SVRDeviceState state)
    {
        ...
        state.deviceVelocity_ = new Vector3(
            linear_velocity.x,
            linear_velocity.y,
            linear_velocity.z
        );
        state.deviceAngularVelocity_ = new Vector3(
            angular_velocity.x,
            angular_velocity.y,
            angular_velocity.z
        );
    ...
    void SVRControllerPose(
        long poll_timestamp,
        int chirality,
        ref svr.SVRInputApi.SVRPose pose,
        ref svr.SVRInputApi.SVRVector3f linear_velocity,
        ref svr.SVRInputApi.SVRVector3f angular_velocity
    )
    {
#if UNITY_IOS || UNITY_VISIONOS
        svr.SVRInputApi.SVRPollDevicePose(
            chirality,
            poll_timestamp,
            ref pose,
            ref linear_velocity,
