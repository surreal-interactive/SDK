...
namespace svr {
    public class SVRInputApi : MonoBehaviour
    {
        ...
        [DllImport("__Internal")]
        public static extern bool SVRQueryDevicePose(long timestamp, int hand_type, ref SVRPose pose, ref SVRVector3f linear_velocity, ref SVRVector3f angular_velocity, IntPtr hand_skeletons);

        [DllImport("__Internal")]
        public static extern void SVRSetButtonCallback(IntPtr callback_ptr);
        
        [DllImport("__Internal")]
        public static extern bool SVRHaptic(int hand_type, int mode, int duration_ms);
